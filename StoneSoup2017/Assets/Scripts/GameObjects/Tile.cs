...
public class Tile : MonoBehaviour {
	...
	//same for if it's just regular old hit.
	...
	//here's just a mod to add to the zIndex in case you need stuff to appear a little on top of other stuff.
	public int zMod;
	...
	protected void moveViaVelocity(Vector2 direction, float speed, float acceleration) {
		if (_body != null) {
			...
			//now we'll z sort based on position
			_sprite.sortingOrder = -((int)transform.position.y + zMod);
		}
	}
