...
public class Player : Tile {
		...
		if (_iFrameTimer <= 0 && !GameManager.instance.gameIsOver) {
			...
			if (amount >= health) {
				if (health > 0) {
					...
					tileThatKilledMe = tileDamagingUs;
					//_anim.GetCurrentAnimatorClipInfo ().SetValue (tileDamagingUs, 0);
				}
				health = 0;
	...
	public void EndGame(){
		GameManager.instance.playerJustDefeated(tileThatKilledMe);
	}
	...
	void FixedUpdate() {
		...
		if (tileWereHolding != null) {
			tileWereHolding.sprite.sortingOrder = sprite.sortingOrder + 1;
		}
	}
